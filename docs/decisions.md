...
Contexto: sin observabilidad accionable, la operacion es reactiva. Sin revision de pilares, las decisiones son tacitas. Sin politica de scaling declarada, la capacidad la ajusta alguien a mano cuando algo se rompe.
Alternativas: solo dashboards (no accionable), monitorear por Slack (fragil), alarmas genericas (ruido).
Tradeoff: cada alarma pide un runbook y un destinatario. Sin eso, se convierte en ruido y se ignora — peor que no tenerla.
